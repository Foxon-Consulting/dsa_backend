...
class DocumentSortingAssistantCrew:
    ...
    def suggest_filename_agent(self) -> Agent:
        return Agent(
            config=self.agents_config["suggest_filename_agent"], verbose=True
        )

    @agent
    def suggest_directory_agent(self) -> Agent:
        return Agent(
            config=self.agents_config["suggest_directory_agent"], verbose=True
        )
    ...
    def suggest_filename_task(self) -> Task:
        return Task(
            config=self.tasks_config["suggest_filename_task"],
            output_file="/logs/suggest_filename_task.md",
        )
