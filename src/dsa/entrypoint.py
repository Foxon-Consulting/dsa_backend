...
def generate_tree(
    starting_directory: Path = Path("./src/dsa/dir"),
) -> List[Path]:
...
def main():
    ...
    if pdf_variable := st.file_uploader("Upload a PDF file", type=["pdf"]):
        ...
        rename_custom_crew = RenameCrew(
            [PDFSearchTool(pdf=str(temp_path_file))], llm
        )
        ...
        sort_custom_crew = SortCrew(
            [PDFSearchTool(pdf=str(temp_path_file))], llm, paths_list
        )
    ...
    if st.button("Are you satisfied with the results?", key="satisfaction"):
        if st.radio("Please confirm:", ("Yes", "No")) == "Yes":
            ...
            st.session_state["messages"].append(
                {
                    "role": "assistant",
                    "content": f"File has been moved to {final_path} and renamed to {final_name}",
                }
            )
            st.chat_message("assistant").write(
                f"File has been moved to {final_path} and renamed to {final_name}"
            )
