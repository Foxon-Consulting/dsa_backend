...
def get_tree(root_folder: Path) -> List[Path]:
    ...
    try:
        if not root_folder.exists():
            ...
            return []  # Retourner une liste vide si le dossier racine n'existe pas
        ...
        return [root_folder] if root_folder.exists() and root_folder.is_dir() else []
    ...
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            try:
                ...
                print(f"File {file_path} is a valid PDF with {page_count} pages")
                return page_count > 0
            except Exception as e:
...
def get_suggested_file_name(source_file_path: Path, llm: LLM = RENAME_OPENAI_API_KEY) -> str:
    ...
    if not is_valid_pdf(source_file_path):
        print(f"File {source_file_path} is not a valid PDF, returning original name")
        return source_file_path.name  # Retourner le nom original en cas de fichier non PDF
...
def get_suggested_path(root_folder: Path, source_file_path: Path, llm: LLM = RENAME_OPENAI_API_KEY) -> str:
    ...
    """
    print(f"Processing file for path suggestion: {source_file_path}, root folder: {root_folder}")

    # Vérifier si le fichier est un PDF valide
    if not is_valid_pdf(source_file_path):
        print(f"File {source_file_path} is not a valid PDF, returning root folder path")
        return str(root_folder)  # Retourner le dossier racine en cas de fichier non PDF
    ...
    try:
        ...
        if not root_folder.exists():
            print(f"Root folder {root_folder} does not exist. Using as root folder.")
        ...
        if not directory_tree:
            print(f"No valid directories found in {root_folder}. Using root folder as default.")
            return str(root_folder)
        ...
        if not suggested_exists:
            print(f"Suggested path '{suggested_path}' is not in the available directory tree.")
            ...
            print(f"No matching directory found. Using root folder: {root_folder}")
            return str(root_folder)
